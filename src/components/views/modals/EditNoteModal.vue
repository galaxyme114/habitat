...
export default {
  ...
  methods: {
    ...
    hide () {
      ...
      this.noteColour = this.note.colour
      this.$emit('close')
    },
    updateColour (value) {
    ...
    async updateNote () {
      try {
        ...
        await api.notes.updateNote(this.note.id, payload)
        this.ok()
        ...
        this.$toasted.success('Note has been updated')
      } catch (error) {
        this.$toasted.error('Note not updated')
      }
