<template>
  <div class="habitat-emoji-wrapper">
    <div
      class="habitat-svg-button"
      :class="{ 'habitat-emoji-reaction': emojiReaction }"
      @click="openEmoji"
    >
    ...
    <template v-if="emojiOpened">
      <transition name="habitat-slide-up" appear>
        <div
          class="habitat-emoji-picker"
          :class="{ 'habitat-picker-reaction': emojiReaction }"
          :style="{
      ...
      setTimeout(() => {
        ...
        if (mobileSize) {
          ...
          const pickerTopPosition =
            channelFooterTop - clientY > this.emojiPickerHeight - 50
