<template>
  <transition-group v-if="!message.deleted" name="habitat-slide-left" tag="span">
    <button
      v-for="(reaction, key) in message.reactions"
      v-show="reaction.length"
      :key="key + 0"
      class="habitat-button-reaction"
      :class="{
        'habitat-reaction-me': reaction.indexOf(currentUserId) !== -1
      }"
      :style="{
