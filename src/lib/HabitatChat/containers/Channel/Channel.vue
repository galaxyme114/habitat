<template>
  <div
    v-show="(isMobile && !showChannelsList) || !isMobile || singleChannel"
    class="habitat-col-messages"
    @touchstart="touchStart"
  >
    <slot v-if="showNoChannel" name="no-channel-selected">
      <div class="habitat-container-center habitat-channel-empty">
        <div>{{ textMessages.CHANNEL_EMPTY }}</div>
      </div>
    </slot>
    ...
    <channel-header
      ...
      :channel="channel"
      @toggle-channels-list="$emit('toggle-channels-list')"
      @channel-info="$emit('channel-info')"
    ...
    <div
      id="messages-list"
      ref="scrollContainer"
      class="habitat-container-scroll"
    ...
    >
      ...
      <div class="habitat-messages-container">
        <div :class="{ 'habitat-messages-hidden': loadingMessages }">
          <transition name="habitat-fade-message">
            <div>
              <div v-if="showNoMessages" class="habitat-text-started">
                <slot name="messages-empty">
                  {{ textMessages.MESSAGES_EMPTY }}
                </slot>
              </div>
              <div v-if="showMessagesStarted" class="habitat-text-started">
          ...
          <transition-group :key="channelId" name="habitat-fade-message" tag="span">
            <div v-for="(m, i) in messages" :key="m.indexId || m._id">
              <message
                ...
                :text-messages="textMessages"
                :channel-footer-ref="$refs.channelFooter"
    ...
    <div v-if="!loadingMessages">
      <transition name="habitat-bounce">
        <div v-if="scrollIcon" class="habitat-icon-scroll" @click="scrollToBottom">
          <transition name="habitat-bounce">
            <div
              v-if="scrollMessagesCount"
              class="habitat-badge-counter habitat-messages-count"
            >
              {{ scrollMessagesCount }}
            </div>
          </transition>
    ...
    <div
      v-show="Object.keys(channel).length && showFooter"
      ref="channelFooter"
      class="habitat-channel-footer"
      :class="{
        'habitat-app-box-shadow': shadowFooter
      }"
    >
      <channel-emojis
      ...
      <div
        class="habitat-box-footer"
        :class="{ 'habitat-box-footer-border': !files.length }"
      >
        <div v-if="showAudio && !files.length" class="habitat-icon-textarea-left">
          <template v-if="isRecording">
            <div
              class="habitat-svg-button habitat-icon-audio-stop"
              @click="stopRecorder"
            >
            ...
            <div class="habitat-dot-audio-record" />

            <div class="habitat-dot-audio-record-time">
              {{ recordedTime }}
            </div>

            <div
              class="habitat-svg-button habitat-icon-audio-confirm"
          ...
          <div v-else class="habitat-svg-button" @click="toggleRecorder(true)">
        ...
        <textarea
          ...
          class="habitat-textarea"
          :class="{
            'habitat-textarea-outline': editedMessage._id
          }"
          :style="{
            'min-height': `20px`,
            'padding-left': `12px`
        ...
        <div class="habitat-icon-textarea">
          <div
            v-if="editedMessage._id"
            class="habitat-svg-button"
            @click="resetMessage"
          >
          ...
          <div
            v-if="showSendIcon"
            class="habitat-svg-button"
            :class="{ 'habitat-send-disabled': isMessageEmpty }"
            @click="sendMessage"
          >
            <slot name="send-icon">
              <svg-icon
...
import ChannelFiles from './ChannelFiles/ChannelFiles'
import ChannelMessageReply from './ChannelMessageReply/ChannelMessageReply'
import ChannelUsersTag from './ChannelUsersTag/ChannelUsersTag'
import ChannelEmojis from './ChannelEmojis/ChannelEmojis'
import ChannelTemplatesText from './ChannelTemplatesText/ChannelTemplatesText'
...
export default {
  ...
  props: {
    ...
    loadFirstChannel: { type: Boolean, required: true },
    messages: { type: Array, required: true },
    channelMessage: { type: String, default: null },
  ...
  computed: {
    channel () {
      return this.channels.find(channel => channel.channelId === this.channelId) || {}
    },
    showNoMessages () {
      return (
    ...
    showNoChannel () {
      const noChannelSelected =
        (!this.channels.length && !this.loadingChannels) ||
        (!this.channel.channelId && !this.loadFirstChannel)
  ...
  watch: {
    ...
    channel: {
      ...
      handler (newVal, oldVal) {
        if (newVal.channelId && (!oldVal || newVal.channelId !== oldVal.channelId)) {
    ...
    channelMessage: {
      immediate: true,
      handler (val) {
        if (val) this.message = this.channelMessage
      }
    },
    messages: {
      deep: true,
  ...
  methods: {
    ...
    getTextareaRef () {
      return this.$refs.channelTextarea
    },
    touchStart (touchEvent) {
      if (this.singleChannel) return
    ...
    touchEnd (touchEvent, posXStart, posYStart) {
      if (touchEvent.changedTouches.length === 1) {
        ...
        if (swippedRight && !swippedVertically) {
          this.$emit('toggle-channels-list')
        }
      }
    },
    onChannelChanged () {
      ...
      if (this.channelMessage) {
        this.message = this.channelMessage
        setTimeout(() => this.onChangeInput())
      }
    ...
    updateFooterList (tagChar) {
      ...
      if (tagChar === '@' && (!this.userTagsEnabled || !this.channel.users)) {
        return
      }
    ...
    selectUserTag (user, editMode = false) {
      ...
      if (!editMode) {
        this.cursorRangePosition =
          position + user.firstName.length + space.length + 1
      }
    ...
    resetMessage (disableMobileFocus = false, initChannel = false) {
      if (!initChannel) {
    ...
    loadMoreMessages () {
      ...
      setTimeout(
        () => {
          ...
          if (this.messagesLoaded || !this.channel.channelId) {
    ...
    editMessage (message) {
      ...
      usertags.forEach(index => {
        ...
        const user = this.channel.users.find(user => user._id === userId)
    ...
    scrollToBottom () {
      setTimeout(() => {
        const element = this.$refs.scrollContainer
        element.classList.add('habitat-scroll-smooth')
        element.scrollTo({ top: element.scrollHeight, behavior: 'smooth' })
        setTimeout(() => element.classList.remove('habitat-scroll-smooth'))
      }, 50)
    },
